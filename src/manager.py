...
class Manager:
        ...
        try:
            ...
            for file in entries:
                if file.endswith('.txt'):
                    temp = pd.read_csv(os.path.join(path_to_data, file), header = None)
                    self.data = pd.concat([self.data,temp])
                if file.endswith('.csv'):
                    temp = pd.read_csv(os.path.join(path_to_data, file), delimiter=';')
                    temp = temp['email']
                    self.data = pd.concat([self.data,temp])
            return self.data
        except FileNotFoundError:
            __class__.log.log_message("File or directory with data not found" , 4)
        except Exception:
            __class__.log.log_message("Something went wrong with loading data ", 4)
    ...
    def manage(self, args):
        ...
        try:
            ...
            self.__class__.log.log_message("Config file not found file should be in config directory" , 4)
        except Exception:
            __class__.log.log_message("Something went wrong with loading config ", 4)
        ...
        if args.find_emails_not_in_logs:
            operator.find_emails_not_in_log(self.data, args.find_emails_not_in_logs)
